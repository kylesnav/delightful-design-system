...
2. **Validate against existing presets.** The three spring presets (snappy, standard, gentle) are already used in the `Documentation/reference/animation-reference.html` demo. Build `spring.js`, then validate that the module produces the same behavior as the inline demo code. If the behavior diverges, the demo is the source of truth.
...
- The `Documentation/reference/animation-reference.html` source code uses `x * 30` on a `[-0.5, 0.5]` normalized range, yielding **±15°** maximum target
