...
The original task brief specifies **±12°** maximum rotation. The `Documentation/reference/animation-reference.html` source code uses `x * 30` on a `[-0.5, 0.5]` normalized range, yielding **±15° maximum target**.
...
The complete existing spec content has been absorbed into this brief — see spec content in this document. The original spec was rated 10/11 in the quality audit. Note: this component does NOT appear in `Documentation/reference/design-reference.html` — it is sourced from `Documentation/reference/animation-reference.html`.
