...
- Existing spring physics implementation: `Documentation/reference/animation-reference.html` inline JS (lines ~4200-4260)
- Spring presets: `SPRING_PRESETS` object in `Documentation/reference/animation-reference.html`
- FLIP concept: standard web animation pattern (First, Last, Invert, Play)
- Particle system: `Documentation/reference/animation-reference.html` canvas demos
- Architecture summary (from original Core Concepts Section 2.3): JS-powered physics (spring.js, flip.js, particles.js), canvas rendering, 23 demos. Browser-only, palette-agnostic.
- Phase 6 deliverables (from original Rebuild Plan): spring.js (core physics + createSpring + 3 presets), flip.js (measureFirst/Last/invert/play), particles.js (canvas + splashEffect), index.js barrel. All respect prefers-reduced-motion.
