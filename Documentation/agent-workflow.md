...
For this project: read your phase prompt from `Documentation/phases/` and the referenced spec files from `Documentation/specs/`. Do not implement from memory — specs are the source of truth. If a spec and the ticket conflict, the spec wins; flag the discrepancy in a comment.
...
For Phases 3, 4, 5, and 7: open the affected pages in a browser and verify visually. Check light mode, dark mode, and reduced-motion behavior. Compare against `Documentation/reference/design-reference.html`. Do not skip this step for phases with visual output.
...
See **Critical Rules** in [`how-to-execute.md`](how-to-execute.md).
