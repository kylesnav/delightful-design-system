...
> **Note:** `.blur-grid` and `.blur-card` here refer to motion utility classes that implement a hover-dim effect. They are not the same as the `blur-grid` layout component, which is deferred to the Roadmap package (see `Documentation/future/roadmap/specs/blur-grid-brief.md`). The motion utility classes define transition and filter behavior; the Roadmap component defines the full layout, markup, and variant structure.
