...
### Keyframes
- [ ] `src/motion/motion.css` exists and is valid CSS
- [ ] All 59 keyframes are present and named correctly
- [ ] Keyframes are wrapped in `@layer component { }` and `@media (prefers-reduced-motion: no-preference) { }`
- [ ] Global reduced-motion override exists in `reset.css`:
  ```css
...
### Animation Classes
- [ ] Each keyframe has a corresponding `.anim-*` utility class
- [ ] Animation classes specify correct duration and easing tokens (`var(--motion-*)`, `var(--ease-*)`)
- [ ] All classes use `animation-fill-mode: both`

### Animation Timing
- [ ] Entrance animations use `var(--motion-base)` (240ms) or faster
- [ ] Exit animations use `var(--motion-fast)` (160ms) or faster
- [ ] Looping animations use appropriate infinite durations
- [ ] Scroll-linked animations use `animation-timeline: scroll()` or `view()` and are gated with `@supports`

### Reduced-Motion Compliance
- [ ] A test page with `prefers-reduced-motion: reduce` shows no visible animation
- [ ] JS-driven behaviors check `prefersReduced` before creating animated elements
- [ ] Splash/particle engine (`pop-anim`, `ripple-anim`) returns early when `prefersReduced` is true
